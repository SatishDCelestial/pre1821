...
public:
	...
	{
		ATLASSERT(m_hPen == NULL);
		m_hPen = ::ExtCreatePen((DWORD)nPenStyle, (DWORD)nWidth, pLogBrush, (DWORD)nStyleCount, lpStyle);
		return m_hPen;
	}
#endif //!UNDER_CE
...
public:
	...
	{
		ATLASSERT(m_hBitmap == NULL);
		m_hBitmap = ::CreateMappedBitmap(_AtlBaseModule.GetResourceInstance(), (INT_PTR)nIDBitmap, (WORD)nFlags, lpColorMap, nMapSize);
		return m_hBitmap;
	}
#endif // UNDER_CE
...
//REVIEW
	...
	{
		ATLASSERT(m_hBitmap != NULL);
		return (DWORD)::SetBitmapBits(m_hBitmap, dwCount, lpBits);
	}
	DWORD GetBitmapBits(DWORD dwCount, LPVOID lpBits) const throw()
	{
		ATLASSERT(m_hBitmap != NULL);
		return (DWORD)::GetBitmapBits(m_hBitmap, (long)dwCount, lpBits);
	}
...
public:
	...
	{
		ATLASSERT(m_hRgn == NULL);
		m_hRgn = ::ExtCreateRegion(lpXForm, (DWORD)nCount, pRgnData);
		return m_hRgn;
	}
#endif //!UNDER_CE
...
// Operations
	...
	{
		ATLASSERT(m_hRgn != NULL);
		return (int)::GetRegionData(m_hRgn, (DWORD)nDataSize, lpRgnData);
	}
};
...
//REVIEW
	...
	{
		ATLASSERT(m_hDC != NULL);
		return ::PolylineTo(m_hDC, lpPoints, (DWORD)nCount);
	}
	BOOL PolyPolyline(const POINT* lpPoints,
		const DWORD* lpPolyPoints, int nCount) throw()
	{
		ATLASSERT(m_hDC != NULL);
		return ::PolyPolyline(m_hDC, lpPoints, lpPolyPoints, (DWORD)nCount);
	}

	BOOL PolyBezier(const POINT* lpPoints, int nCount) throw()
	{
		ATLASSERT(m_hDC != NULL);
		return ::PolyBezier(m_hDC, lpPoints, (DWORD)nCount);
	}
	BOOL PolyBezierTo(const POINT* lpPoints, int nCount) throw()
	{
		ATLASSERT(m_hDC != NULL);
		return ::PolyBezierTo(m_hDC, lpPoints, (DWORD)nCount);
	}
#endif //!UNDER_CE
...
// Text Functions
	...
	BOOL ExtTextOut(int x, int y, UINT nOptions, LPCRECT lpRect,
		LPCTSTR lpszString, int nCount = -1, LPINT lpDxWidths = NULL) throw()
	{
		ATLASSERT(m_hDC != NULL);
		if(nCount == -1)
			nCount = lstrlen(lpszString);
...
#ifndef UNDER_CE
	DWORD GetTabbedTextExtent(LPCTSTR lpszString, int nCount,
		int nTabPositions, LPINT lpnTabStopPositions) const throw()
	{
		ATLASSERT(m_hDC != NULL);
		if(nCount == -1)
			nCount = lstrlen(lpszString);
		return (DWORD)::GetTabbedTextExtent(m_hDC, lpszString, nCount, nTabPositions, lpnTabStopPositions);
	}
...
// Font Functions
	...
	DWORD GetKerningPairs(int nPairs, LPKERNINGPAIR lpkrnpair) const throw()
	{
		ATLASSERT(m_hDC != NULL);
		return ::GetKerningPairs(m_hDC, (DWORD)nPairs, lpkrnpair);
	}
...
/**/						::SelectObject(pDC->m_hDC, hObjOld);
		...
		default:
			::PlayMetaFileRecord(hDC, pHandleTable, pMetaRec, (DWORD)nHandles);
			break;
		}
