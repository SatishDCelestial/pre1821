...
public:
    CPenDC (HDC hDC, COLORREF crColor = CLR_NONE, int nWidth = VsUI::DpiHelper::LogicalToDeviceUnitsX(1));
   ~CPenDC ();
...
//  CFontDC (HDC hDC, BYTE nStockFont, COLORREF crText = CLR_DEFAULT);
	CFontDC (HDC hDC, HFONT hFont, COLORREF crText = CLR_DEFAULT); // does not copy or delete hFont
   ~CFontDC ();
...
public:
	CFormatDC(HDC hDC, bool bBold, bool bItalic, bool bUnderline);
	~CFormatDC();
};
...
//
#if defined(DB_EMPTY)
#undef DB_EMPTY		// defined in vs2005 ocdb.h with different value
#endif // DB_EMPTY
