...
// we link statically to MFC so dllimport causes link warning
...
{
    CRect rcTBItem;
    int nTBIndex = CommandToIndex ((UINT)nID);
...
CSize CToolBarXP::CalcDynamicLayout (int nLength, DWORD dwMode)
{
    ...
    if ( dwMode & LM_COMMIT )
    {
        ...
        {
            VERIFY(DefWindowProc (TB_GETBUTTON, (WPARAM)i, (LPARAM)&tbbutton));
            ...
            else
            {
                ...
                VERIFY (DefWindowProc (TB_GETITEMRECT, (WPARAM)i, (LPARAM)&rcControl));
...
void CToolBarXP::OnPaint ()
{
    ...
    for ( int i = 0; i < nCount; i++ )
    {
        ...
        CRect rcButton;
        VERIFY(DefWindowProc (TB_GETITEMRECT, (WPARAM)i, (LPARAM)&rcButton));
