...
HLSCOLOR RGB2HLS (COLORREF rgb)
{
    ...
    if ( maxval != minval )
    { 
        float rnorm = float(maxval - GetRValue(rgb)) / mdiff;
        float gnorm = float(maxval - GetGValue(rgb)) / mdiff;
        float bnorm = float(maxval - GetBValue(rgb)) / mdiff;
...
{
    float hue        = float(((float)(int)HLS_H(hls)*360)/255.0f);
    float luminance  = float(HLS_L(hls)/255.0f);
    float saturation = float(HLS_S(hls)/255.0f);
...
///////////////////////////////////////////////////////////////////////////////
///////////////////////////////////////////////////////////////////////////////
CPenDC::CPenDC (HDC hDC, COLORREF crColor, int nWidth) : m_hDC (hDC), mWidth(nWidth)
{
    VERIFY(m_pen.CreatePen (PS_SOLID, mWidth, crColor));
...
CFontDC::CFontDC (HDC hDC, HFONT hFont, COLORREF crText)
        : m_hFont (NULL), m_hDC(hDC), m_hDefFont (NULL), m_crTextOld (CLR_NONE)
{
    *this = hFont;
...
/////////////////////////////////////////////////////////////////////////////
/////////////////////////////////////////////////////////////////////////////
CFormatDC::CFormatDC(HDC hDC, bool bBold, bool bItalic, bool bUnderline) : m_hDC(hDC), m_hDefFont(NULL)
{
	LOGFONT lf;
		...
		)
	{
		...
		m_fontFormat.CreateFontIndirect(&lf);
		m_hDefFont = (HFONT)::SelectObject(m_hDC, m_fontFormat);
	}
}
...
/////////////////////////////////////////////////////////////////////////////
/////////////////////////////////////////////////////////////////////////////
CDimDC::CDimDC(HDC hDC, bool bDim) : m_hDC(hDC), m_crTextOld(CLR_NONE)
{
	if (bDim)
	{
		auto fgColor = ::GetTextColor(m_hDC);

		// HACK - can't trust GetBkColor.  only dim towards black or white
		//auto bgColor = ::GetBkColor(m_hDC);
		int sum = (int)GetRValue(fgColor) + (int)GetGValue(fgColor) + (int)GetBValue(fgColor);
		COLORREF bgColor = (sum > 0x7f * 3) ? 0 : 0x00ffffffu;

		auto dimColor = ThemeUtils::InterpolateColor(bgColor, fgColor, (float)Psettings->mDimmedMenuItemOpacityValue / 100.0f);
		::SetTextColor(m_hDC, dimColor);
		m_crTextOld = fgColor;
	}
}
...
void CDrawButton::Draw (CDC* pDC, DWORD wStyle) const
{
    ...
    if ( DB_GETTYPE(wStyle) == DB_UPDOWN )
    {
        if ( !DB_ISPRESSED2(wStyle) )
        {
            ...
            else
            {
                ...
                pDC->DrawFrameControl (rcBtn, DFC_BUTTON, DFCS_BUTTONPUSH|(DB_ISPRESSED1(wStyle) ? DFCS_PUSHED : 0u));
            }
        }
        ...
        if ( !DB_ISPRESSED1(wStyle) )
        {
            ...
            else
            {
                ...
                pDC->DrawFrameControl (rcBtn, DFC_BUTTON, DFCS_BUTTONPUSH|(DB_ISPRESSED2(wStyle) ? DFCS_PUSHED : 0u));
            }
        }
        if ( DB_ISFLAT(wStyle) )
        {
    ...
    else
    {
        ...
        {
            pDC->DrawFrameControl (rc, DFC_BUTTON, DFCS_BUTTONPUSH|(DB_ISPRESSED(wStyle) ? DFCS_PUSHED : 0u));
        }
...
{
    ::SendMessage (::GetParent (hWnd), WM_COMMAND, MAKEWPARAM (GetDlgCtrlID (hWnd), s_wNotify), (LPARAM)hWnd);
}
...
DWORD CDrawButton::Click (CWnd* pWnd, CPoint pt, UINT nIDRepeat) const
{
    ...
    if ( DB_GETTYPE(m_wStyle) == DB_UPDOWN )
    {
        ...
        if ( nIDRepeat != 0 )
        {
            ...
            if ( !pWnd->SetTimer (nIDRepeat, 100u, &CDrawButton_TimerProcRepeat) )
            {
                nIDRepeat = 0;
            }
        }
...
void CDrawButton::CheckForMouseOver (CWnd* pWnd, CPoint pt)
{
    if ( DB_ISFLAT(m_wStyle) )
    {
        ...
        {
            if ( pWnd->SetTimer (54321, 300u, &CDrawButton_TimerProcMouseOver) )
            {
                m_wStyle |= DB_OVER;
