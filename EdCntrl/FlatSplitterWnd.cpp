...
CFlatSplitterWnd::CFlatSplitterWnd()
{
	...
	m_cxSplitterGap = m_cySplitterGap = 1 + 1 + 1;
...
void CFlatSplitterWnd::DeferClientPos(AFX_SIZEPARENTPARAMS* lpLayout, CWnd* pWnd, int x, int y, int cx, int cy,
                                      BOOL bScrollBar)
{
	ASSERT(pWnd != NULL);
	...
	if (bScrollBar)
	{
		...
		pWnd->ModifyStyle(bNeedBorder ? 0u : 1u, bNeedBorder ? 1u : 0u);
	}
...
void CFlatSplitterWnd::RecalcLayout()
{
	...
	if (m_bHasHScroll && m_bHasVScroll)
	{
		...
		if (pScrollBar->ModifyStyle(DWORD(SBS_SIZEGRIP | SBS_SIZEBOX),
	...
	if (m_bHasHScroll)
	{
		...
		for (int col = 0; col < m_nCols; col++)
		{
			...
			ASSERT(pScrollBar != NULL);
			int cx2 = m_pColInfo[col].nCurSize;
			if (col == 0 && m_nCols < m_nMaxCols)
				x += cxSplitterBox, cx2 -= cxSplitterBox;
			DeferClientPos(&layout, pScrollBar, x, y, cx2, cy, TRUE);
			x += cx2 + m_cxSplitterGap;
		}
	}
	...
	if (m_bHasVScroll)
	{
		...
		for (int row = 0; row < m_nRows; row++)
		{
			...
			ASSERT(pScrollBar != NULL);
			int cy2 = m_pRowInfo[row].nCurSize;
			if (row == 0 && m_nRows < m_nMaxRows)
				y += cySplitterBox, cy2 -= cySplitterBox;
			DeferClientPos(&layout, pScrollBar, x, y, cx, cy2, TRUE);
			y += cy2 + m_cySplitterGap;
		}
	}
	...
	// BLOCK: Reposition all the panes
	{
		...
		for (int col = 0; col < m_nCols; col++)
		{
			...
			for (int row = 0; row < m_nRows; row++)
			{
				...
				DeferClientPos(&layout, pWnd, x, y, cx2, cy2, FALSE);
				y += cy2 + m_cySplitterGap;
			}
			x += cx2 + m_cxSplitterGap;
		}
