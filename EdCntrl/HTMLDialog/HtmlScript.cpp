...
const CString GetSystemErrorMessage(HRESULT dwError)
{
	CString strError;
	LPTSTR lpBuffer;
...
bool CHtmlScript::CallJScript2(const CString strFunc, const CStringArray& paramArray, CString *sResult)
{
   ...
   psa = SafeArrayCreateVector( VT_VARIANT, 0, (ULONG)arraySize);
   if(psa == NULL){
      return false;
   }   
	
	for(INT_PTR i = 0; i < arraySize; i++)
	{
		VariantInit(&var);
		V_VT(&var) = VT_BSTR;
		CComBSTR bstr = (const char *)paramArray.GetAt(arraySize - 1 - i); // back reading
