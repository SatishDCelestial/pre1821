...
CHtmlDialog::CHtmlDialog( UINT nID_Dlg, CWnd* pParent,UINT nID_HTML, UINT n_ID_static): 
	CDialog(nID_Dlg, pParent), CHtml_Host_Handlers(), 
...
// Since the SetWindowPos was commented out, all of this logic is not used, 
// If params was null, the atoi(csCmd(0+1)) could cause a problem.
// Comment out for case=26741
// 
// 		{
// 			UINT cx, cy = 100, x;
...
//	m_HtmlCtrl.SetWindowPos(NULL, 0,0, r.Width(), r.Height(), 0);
	...
	CString title(m_HtmlCtrl.m_html_Title);
	if (title == "Tip of the Day")
	{
		// [case: 2324]
		title = IDS_APPNAME + CString(" ") + title;
	}
	SetWindowText(title);
}
