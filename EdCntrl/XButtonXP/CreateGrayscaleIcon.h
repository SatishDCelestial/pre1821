...
HICON CreateGrayscaleIcon(HICON hIcon, COLORREF* pPalette)
{
	...
	if (::GetIconInfo(hIcon, &icInfo))
	{
		if (::GetDIBits(hdc, icInfo.hbmColor, 0, 0, NULL, &bmpInfo, 
				DIB_RGB_COLORS) != 0)
		{
			const size_t size = sizeof(BITMAPINFOHEADER) + sizeof(RGBQUAD) * 
			...
			LPDWORD lpBits = NULL;
			if (::GetDIBits(hdc, icInfo.hbmColor, 0, (DWORD)bmpInfo.bmiHeader.biHeight, 
					lpBits, pBmpInfo, DIB_RGB_COLORS) != 0)
			{
				bmpInfo.bmiHeader.biPlanes = 1;
				bmpInfo.bmiHeader.biBitCount = 32;
				bmpInfo.bmiHeader.biSizeImage = DWORD(bmpInfo.bmiHeader.biWidth * 4 * 
				...
				if (lpBits && ::GetDIBits(hdc, icInfo.hbmColor, 0, (UINT)sz.cy, 
						lpBits, &bmpInfo, DIB_RGB_COLORS) != 0)
				{
					...
					for (UINT i = 0; i < c1; i++)
					{
						int dwBits = lpBitsPtr[0] + lpBitsPtr[1] + lpBitsPtr[2];
					...
					{
						::SetDIBits(hdc, icGrayInfo.hbmColor, 0, (DWORD)sz.cy, lpBits, 
