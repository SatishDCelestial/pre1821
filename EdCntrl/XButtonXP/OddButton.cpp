...
void COddButton::SetDefID(CDialog* pDialog, const UINT nID)
{
	...
	const UINT nPrevID = (HIWORD(dwPrevDefID) == DC_HASDEFID) ? LOWORD(dwPrevDefID) : 0u;
	...
	LRESULT lRes = (nPrevID == 0) ? (LRESULT)0 : pDialog->SendDlgItemMessage((int)nPrevID, WM_GETDLGCODE);
	if( (lRes & DLGC_BUTTON) && 
		(lRes & DLGC_DEFPUSHBUTTON) )
	{
		pDialog->SendDlgItemMessage((int)nPrevID, BM_SETSTYLE,
	...
	lRes = (nID == 0) ? 0 : pDialog->SendDlgItemMessage((int)nID, WM_GETDLGCODE);
	if( lRes & DLGC_BUTTON )
	{
			...
			(lResFocus & (DLGC_DEFPUSHBUTTON | DLGC_UNDEFPUSHBUTTON)) )
		{
			...
			{
				pDialog->SendDlgItemMessage((int)nID, BM_SETSTYLE,
					BS_PUSHBUTTON, MAKELPARAM(TRUE, 0));
			}
