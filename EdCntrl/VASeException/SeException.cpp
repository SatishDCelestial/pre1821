...
#define CASE(nSeCode, CsString)                                                                                        \
	case EXCEPTION_##nSeCode:                                                                                          \
		CString__FormatA(CsString, _T("Exception %s (0x%.8lx) at address 0x%.8p."), _T(#nSeCode), EXCEPTION_##nSeCode, \
...
{
	ASSERT(lpszError != NULL && AfxIsValidString(lpszError, (int)nMaxError));
	...
	{
		lstrcpyn(lpszError, strMessage, (int)nMaxError);
		return TRUE;
	}
}
