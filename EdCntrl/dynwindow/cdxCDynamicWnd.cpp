...
#else
	for(HWND hwnd = ::GetWindow(m_pWnd->m_hWnd,GW_CHILD); hwnd; hwnd = ::GetWindow(hwnd,GW_HWNDNEXT))
#endif
	{
...
// OverrideControlPosition
// -----------------------------------------------------------------------
// Specify newPos in terms of previous pos (call GetControlPosition).
// DO NOT use windows to get window rect or you'll end up in hateful pain.
// 
bool
cdxCDynamicWnd::OverrideControlPosition(HWND hwnd, Position newPos)
{
	if (!IsWindow())
	{
		ASSERT(IsWindow());
	...
	if (!::IsWindow(hwnd))
	{
		TRACE(_T("*** NOTE[cdxCDynamicWnd::OverrideControlPosition()]: Handle 0x%p is not a valid window.\n"), hwnd);
		return false;
	}

	m_Map.SetAt(hwnd, newPos);
...
{
	// VADEBUGPRINT("#DPI cdxCDynamicWnd::UpdateControlPosition");
	...
	if(!pli || !pli->IsInitial())
	{
		...
		try
		{
			...
			if(DoMoveCtrl(hwnd,(UINT)::GetDlgCtrlID(hwnd),rectNew,*pli) &&
#if !defined(_WIN32_WCE)
				(rectNew != wpl.rcNormalPosition) )
#else
				(rectNew != r) )
...
void cdxCDynamicWnd::Layout(bool dpiChanged /*= false*/)
{
	// VADEBUGPRINT("#DPI cdxCDynamicWnd::Layout");
	...
	{
#endif
		HDWP							hdwp		=	::BeginDeferWindowPos((int)pli->m_nCtrlCnt);
		HWND							hwnd;
		bool							bRepeat;
		CRect							rectNew;
		UINT							id;
#if !defined(_WIN32_WCE)
		WINDOWPLACEMENT			wpl;
		DWORD							swpFlags	=	SWP_NOACTIVATE|SWP_NOOWNERZORDER|SWP_NOZORDER|(!(m_nFlags & flSWPCopyBits) ? SWP_NOCOPYBITS : 0u);
#else
		RECT r;
		DWORD							swpFlags	=	SWP_NOACTIVATE|SWP_NOOWNERZORDER|SWP_NOZORDER/*|(!(m_nFlags & flSWPCopyBits) ? SWP_NOCOPYBITS : 0u)*/;
#endif
...
{
	// VADEBUGPRINT("#DPI cdxCDynamicWnd::DoInitWindow size: " << szInitial.cx << " " << szInitial.cy);
	...
	if(pLast = __getDynMap(pLast))
	{
		...
		for(pEntry = pLast; pEntry->type != __end; ++pEntry)
		{
			...
			if((pEntry->id != DYNAMIC_MAP_DEFAULT_ID)
				&& !( hwnd = ::GetDlgItem(m_pWnd->m_hWnd, (int)pEntry->id) ))
			{
				TRACE(_T("*** NOTE[cdxCDynamicWnd::DoInitWindow()]: Dynamic map initialization: There's no control with the id 0x%x !\n"),pEntry->id);
				continue;
			}
...
{
	// VADEBUGPRINT("#DPI cdxCDynamicWnd::DoOnSize");
...
void cdxCDynamicWnd::DoOnDPIChange(UINT triggerMessage, UINT oldDpi, UINT newDpi)
{
	// VADEBUGPRINT("#DPI cdxCDynamicWnd::DoOnDPIChange " << triggerMessage);
	...
	{
		{
			auto* mapPos = m_Map.PGetFirstAssoc();
			while (mapPos != nullptr)
			{
				if (::IsWindow(mapPos->key))
				{
					WindowScaler::ScaleRect(mapPos->value, scale);
					WindowScaler::ScaleSize(mapPos->value.m_szMin, scale);
				}

				mapPos = m_Map.PGetNextAssoc(mapPos);
			}
		}

		WindowScaler::ScaleSize(m_szInitial, scale);
		WindowScaler::ScaleSize(m_szMin, scale);
	}

	Layout();
}

void cdxCDynamicWnd::ResetLayout()
{
	if (!IsWindow())
		return;
	...
	while (mapPos != nullptr)
	{
		if (::IsWindow(mapPos->key))
		{
			if (::GetWindowRect(mapPos->key, &mapPos->value))
				Window()->ScreenToClient(&mapPos->value);
		}

		mapPos = m_Map.PGetNextAssoc(mapPos);
	}

	m_szInitial = GetCurrentClientSize();
}
...
{
	// VADEBUGPRINT("#DPI cdxCDynamicWnd::DoOnSizing");
...
void cdxCDynamicWnd::StartAntiFlickering(bool bIsBotRight)
{
	if(IsWindow() && m_nMyTimerID)
	{
		...
		{
			::SetClassLong(*m_pWnd,GCL_STYLE,(LONG)dw);
...
{
	// VADEBUGPRINT("#DPI cdxCDynamicWnd::DoOnTimer");
	...
	if(IsWindow() && (nIDEvent == m_nMyTimerID))
	{
		...
		if(m_bIsAntiFlickering)
		{
			::SetClassLong(*m_pWnd,GCL_STYLE,(LONG)m_dwClassStyle);
			m_bIsAntiFlickering	=	false;
		}
	}
}
...
{
	// VADEBUGPRINT("#DPI cdxCDynamicWnd::DoOnGetMinMaxInfo");
...
void cdxCDynamicWnd::NoSize() {
	m_pSizeIcon->ShowWindow(SW_HIDE);
	nosize = true;
}

bool cdxCDynamicWnd::CurrentToInitial(CPoint& pt)
{
	if (IsWindow())
	{
		auto current = GetCurrentClientSize();
		pt.x = WindowScaler::Transform(current.cx, m_szInitial.cx, pt.x);
		pt.y = WindowScaler::Transform(current.cy, m_szInitial.cy, pt.y);
		return true;
	}
	return false;
}

bool cdxCDynamicWnd::CurrentToInitial(CRect& rect)
{
	if (IsWindow())
	{
		auto current = GetCurrentClientSize();
		rect.left = WindowScaler::Transform(current.cx, m_szInitial.cx, rect.left);
		rect.right = WindowScaler::Transform(current.cx, m_szInitial.cx, rect.right);
		rect.top = WindowScaler::Transform(current.cy, m_szInitial.cy, rect.top);
		rect.bottom = WindowScaler::Transform(current.cy, m_szInitial.cy, rect.bottom);
		return true;
	}
	return false;
}

bool cdxCDynamicWnd::InitialToCurrent(CPoint& pt)
{
	if (IsWindow())
	{
		auto current = GetCurrentClientSize();
		pt.x = WindowScaler::Transform(m_szInitial.cx, current.cx, pt.x);
		pt.y = WindowScaler::Transform(m_szInitial.cy, current.cy, pt.y);
		return true;
	}
	return false;
}

bool cdxCDynamicWnd::InitialToCurrent(CRect& rect)
{
	if (IsWindow())
	{
		auto current = GetCurrentClientSize();
		rect.left = WindowScaler::Transform(m_szInitial.cx, current.cx, rect.left);
		rect.right = WindowScaler::Transform(m_szInitial.cx, current.cx, rect.right);
		rect.top = WindowScaler::Transform(m_szInitial.cy, current.cy, rect.top);
		rect.bottom = WindowScaler::Transform(m_szInitial.cy, current.cy, rect.bottom);
		return true;
	}
	return false;
}
