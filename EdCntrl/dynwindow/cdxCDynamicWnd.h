...
public:
	...
	public:
		...
		void operator=(const CRect & rectInitial) { *((CRect*)this) = rectInitial; }
...
public:
	...
	bool CurrentToInitial(CPoint& pt);
	bool CurrentToInitial(CRect& rect);
	bool InitialToCurrent(CPoint& pt);
	bool InitialToCurrent(CRect& rect);
	...
	bool OverrideControlPosition(HWND hwnd, Position newPos);
	...
	virtual void Layout(bool dpiChanged = false);
...
protected:
	...
	void DoOnDPIChange(UINT triggerMessage, UINT oldDpi, UINT newDpi);
#if !defined(_WIN32_WCE)
...
private:
	// DON'T USE
	DECLARE_CDX_HIDDENFUNC( cdxCDynamicWnd(const cdxCDynamicWnd & w) ) { (void)w; ASSERT(false); }
	void DECLARE_CDX_HIDDENFUNC( operator=(const cdxCDynamicWnd & w) ) { (void)w; ASSERT(false); }
	// helpers
...
protected:
	virtual const __dynEntry * DECLARE_CDX_HIDDENFUNC( __getDynMap(const __dynEntry *pLast) ) const { (void)pLast; return NULL; }
...
inline bool cdxCDynamicWnd::AddSzControl(HWND hwnd, const SBYTES & bytes, const CSize & szMin, bool bReposNow)
{
	...
	return AddSzControl(hwnd,Position(rect/*wpl.rcNormalPosition*/,bytes,szMin),bReposNow);
#endif
}
...
inline void cdxCDynamicWnd::_translate(Mode md, SBYTE & b1, SBYTE & b2)
{
	switch(md)
	{
		...
		case	mdHalfsize	:	b1	=	0;	b2	=	50;	break;
		case	mdHalfpossize	:	b1	=	50;	b2	=	100;	break;
	}
}
...
inline HWND cdxCDynamicWnd::GetSafeChildHWND(UINT nID)
{
	...
	HWND	h	=	::GetDlgItem(m_pWnd->m_hWnd, (int)nID);
	ASSERT(h!=0);
	return h;
}
