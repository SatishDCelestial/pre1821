...
void pfx_chk (const char * word, int len, struct affent* ep, int num)
{
	...
	if (tlen > 0 &&  (aent->appndl == 0 ||  
	    ...
	    if (aent->stripl) strcpy_s (tword, _countof(tword), aent->strip);
	    strcat_s((tword + aent->stripl), _countof(tword) - aent->stripl, (word + aent->appndl));
              ...
              int num, struct affent * pfxent, int cpflag)
{
	...
	if (tlen > 0  &&  (aent->appndl == 0 ||  
	    ...
	    strcpy_s (tword, _countof(tword), word);
	    cp = (unsigned char *) (tword + tlen);
	    if (aent->stripl) {
	       strcpy_s ((char *)cp, _countof(tword) - tlen, aent->strip);
	       tlen += aent->stripl;
	       cp = (unsigned char *)(tword + tlen);
	    } else *cp = '\0';
    ...
    for (aent = ep, i = num; i > 0; aent++, i--) {
        ...
        if ((len > aent->stripl) && (len >= aent->numconds)) {
	      ...
	      /* we have a match so add prefix */
              tlen = 0;
              if (aent->appndl) {
	          strcpy_s(tword, _countof(tword),aent->appnd);
                  tlen += aent->appndl;
               } 
               pp = tword + tlen;
               strcpy_s(pp, _countof(tword) - tlen, (word + aent->stripl));
...
void suf_add (const char * word, int len, struct affent * ep, int num)
{
	...
	if (cond < 0) {
	  ...
	  if (aent->stripl) {
          ...
          pp = (tword + tlen);
          if (aent->appndl) {
	       strcpy_s (pp, _countof(tword) - tlen, aent->appnd);
	       tlen += aent->stripl;
	  } else *pp = '\0';
