...
AffixMgr::AffixMgr(const wchar_t * affpath, HashMgr* ptr) 
{
  ...
  if (parse_file(affpath)) {
     fprintf(stderr,"Failure loading aff file %ws\n",affpath);
     fflush(stderr);
  }
}
...
int  AffixMgr::parse_file(const wchar_t * affpath)
{

  // io buffers
  char line[(MAXLNLEN*kUtf8Expansion)+1];
  ...
  // open the affix file
  FILE * afflst;
  afflst = _wfopen(affpath, L"r");
  if (!afflst) {
    fprintf(stderr,"Error - could not open affix description file %ws\n",affpath);
    return 1;
  }
  ...
  // affix data structures
    ...
    while (fgets(line,MAXLNLEN,afflst)) {
       mychomp(line, MAXLNLEN*kUtf8Expansion);
...
struct hentry * AffixMgr::compound_check (const char * word, int len, char compound_flag)
{
	...
	if ((rv) && (TESTAFF(rv->astr, compound_flag, rv->alen))) {
	    ...
	    rv = affix_check((word+i),strlen_i(word+i));
	    if ((rv) && (TESTAFF(rv->astr, compound_flag, rv->alen))) {
		free(st);
		return rv;
	    }
	    rv = compound_check((word+i),strlen_i(word+i),compound_flag); 
	    if (rv) {
		free(st);
		return rv;
	    }
...
int  AffixMgr::parse_reptable(char * line, FILE * af)
{
   ...
   for (int j=0; j < numrep; j++) {
        ...
        mychomp(nl, MAXLNLEN*kUtf8Expansion);
        tp = nl;
        i = 0;
        reptable[j].pattern = NULL;
        reptable[j].replacement = NULL;
           ...
           if (*piece != '\0') {
               switch(i) {
                  ...
                  case 1: { maptable[j].set = mystrdup(piece); 
		            maptable[j].len = strlen_i(maptable[j].set);
                            break; }
		  default: break;
               }
               i++;
