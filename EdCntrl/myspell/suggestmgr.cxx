...
SuggestMgr::SuggestMgr(const char * tryme, size_t maxn, 
                       AffixMgr * aptr)
{
...
int SuggestMgr::suggest(char** wlst, int ns, const char * word)
{
    ...
    if ((nsug < (int)maxSug) && (nsug > -1))
      nsug = swapchar(wlst, word, nsug);

    // perhaps we made chose the wrong char from a related set
	const int max_recursion_level = 18;
    if ((nsug < (int)maxSug) && (nsug > -1))
      nsug = mapchars(wlst, word, nsug, max_recursion_level);
...
// [case: 140854]
bool gRunMapRelated = true;
DWORD gMapRelatedStart = 0;

// suggestions for when chose the wrong char out of a related set
int SuggestMgr::mapchars(char** wlst, const char * word, int ns, int max_recursion_level)
{
  ...
  gRunMapRelated = true;
  gMapRelatedStart = GetTickCount();
  ns = map_related(word, 0, wlst, ns, maptable, nummap, max_recursion_level);
  return ns;
}


int SuggestMgr::map_related(const char * word, int i, char** wlst, int ns, const mapentry* maptable, int nummap, int max_recursion_level) 
{
  ...
  if (c == 0) {
      ...
      if ((cwrd) && check(word,strlen_i(word))) {
	  if (ns < (int)maxSug) {
  ...
  int in_map = 0;
  for (int j = 0; j < nummap && gRunMapRelated; j++) {
    if (strchr(maptable[j].set,c) != 0) {
      in_map = 1;
      char * newword = _tcsdup(word);
      for (int k = 0; k < maptable[j].len && gRunMapRelated; k++) {
		*(newword + i) = *(maptable[j].set + k);
		ns = map_related(newword, (i+1), wlst, ns, maptable, nummap, max_recursion_level);
      }
      free(newword);
    }

	if ((GetTickCount() - gMapRelatedStart) > 2000)
	{
		gRunMapRelated = false;
		return ns;
	}
  }
  if (!in_map) {
     i++;
     ns = map_related(word, i, wlst, ns, maptable, nummap, max_recursion_level);
  }
  return ns;
}
...
int SuggestMgr::replchars(char** wlst, const char * word, int ns)
{
  ...
  for (int i=0; i < numrep; i++ ) {
	  ...
	  strcpy(candidate+(r-word)+lenr, r+lenp);
          ...
          if ((cwrd) && check(candidate,strlen_i(candidate))) {
	      if (ns < (int)maxSug) {
...
int SuggestMgr::twowords(char ** wlst, const char * word, int ns)
{
    ...
    for (p = candidate + 1;  p[1] != '\0';  p++) {
       p[-1] = *p;
       *p = '\0';
       if (check(candidate,strlen_i(candidate))) {
	 if (check((p+1),strlen_i(p+1))) {
	    *p = ' ';
	    if (ns < (int)maxSug) {
                wlst[ns] = mystrdup(candidate);
...
int SuggestMgr::ngsuggest(char** wlst, char * word, HashMgr* pHMgr)
{
  ...
  int col = -1;
  while ((hp = pHMgr->walk_hashtable(col, hp)) != NULL) {
...
//      lval = sc;
  ...
  int thresh = 0;
  char * mw = NULL;
  for (size_t sp = 1; sp < 4; sp++) {
     mw = _tcsdup(word);
     for (size_t k=sp; k < n; k+=4) *(mw + k) = '*';
     thresh = thresh + ngram((int)n, word, mw, NGRAM_ANY_MISMATCH);
     free(mw);
  }
  mw = NULL;
  thresh = thresh / 3;
  ...
  for (i = 0; i < MAX_ROOTS; i++) {
	...
	int nw = pAMgr->expand_rootword(glst, MAX_WORDS, rp->word, rp->wlen,
                                        rp->astr, rp->alen);
        for (int k = 0; k < nw; k++) {
           sc = ngram((int)n, word, glst[k].word, NGRAM_ANY_MISMATCH);
           if (sc > thresh) {
              if (sc > gscore[lp]) {
