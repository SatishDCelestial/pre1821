...
void mychomp(char * s, int buflen)
{
  int k = strlen_i(s);
  if ((k > 0) && ((*(s+k-1)=='\r') || (*(s+k-1)=='\n'))) *(s+k-1) = '\0';
  if ((k > 1) && (*(s+k-2) == '\r')) *(s+k-2) = '\0';

  if (s && k && !::CanReadAsUtf8(s))
  {
	  // [case: 132797]
	  // convert string from dictionary file to unicode, using active codepage.
	  // this block will not be entered if user converts the dictionary to utf8.
	  // can't convert from mbcs codepage to utf8, so convert to wide then narrow to utf8.
	  WTString tmpwt(::MbcsToWide(s, k, (int)::GetACP()));
	  // and copy back to local buf
	  if (tmpwt.GetLength() < buflen)
		::strncpy_s(s, (size_t)buflen, tmpwt.c_str(), (size_t)tmpwt.GetLength());
  }
}
...
void enmkallcap(char * d, const char * p, const char * encoding)
{
  ...
  while (*p != '\0') { 
    *d++ = (char)csconv[((unsigned char) *p)].cupper;
    p++;
  }
  *d = '\0';
}
...
void enmkallsmall(char * d, const char * p, const char * encoding)
{
  struct cs_info * csconv = get_current_cs(encoding);
  while (*p != '\0') { 
    *d++ = (char)csconv[((unsigned char) *p)].clower;
    p++;
  }
  *d = '\0';
}
...
void enmkinitcap(char * d, const char * p, const char * encoding)
{
  ...
  if (*p != '\0') *d= (char)csconv[((unsigned char)*p)].cupper;
}
...
{
  while (*p != '\0') { 
    *p = (char)csconv[((unsigned char) *p)].cupper;
    p++;
  }
}
...
{
  while (*p != '\0') { 
    *p = (char)csconv[((unsigned char) *p)].clower;
    p++;
  }
}
...
{
  if (*p != '\0') *p = (char)csconv[((unsigned char)*p)].cupper;
}
