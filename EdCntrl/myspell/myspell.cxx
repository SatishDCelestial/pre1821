...
MySpell::MySpell(const wchar_t * affpath, const wchar_t * dpath)
{
    encoding = NULL;
    csconv = NULL;
...
int MySpell::spell(const char * word)
{
  ...
  int captype = 0;
  int abbv = 0;
  wl = (size_t)cleanword(cw, word, &captype, &abbv);
  if (wl == 0) return 1;
...
char * MySpell::check(const char * word)
{
  ...
  if ((he == NULL) && (pAMgr)) {
     ...
     he = pAMgr->affix_check(word, strlen_i(word));

     // try check compound word
     if ((he == NULL) && (pAMgr->get_compound())) {
          he = pAMgr->compound_check(word, strlen_i(word), (pAMgr->get_compound())[0]);
     }
...
void MySpell::AddWord(const char *word) {
	if(!word || !word[0])
		return;
	if(pHMgr->lookup(word))
		return;

	pHMgr->add_word(word, strlen_i(word), NULL, 0);
}
